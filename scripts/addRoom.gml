...
ds_map_add(lastRoom, "x1", argument0 );
ds_map_add(lastRoom, "y1", argument1);
...
for (var k = 0; k < argument3; k++) {
    for (var l = 0; l < argument2; l++) {
        if (k == 0) {
            ds_map_replace(ds_grid_get(dungeonBlocks, argument0+l, argument1+k), "blockType",1);
        }
            ...
            ds_map_replace(ds_grid_get(dungeonBlocks, argument0+l, argument1+k), "blockType", 2); 
    }
}
